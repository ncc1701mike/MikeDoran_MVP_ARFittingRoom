...
namespace NeutronCat.SolarSystem
{
    ...
    public class SimpleViewer : MonoBehaviour
    {
        ...
        public void LateUpdate()
        {
            ...
            float _camRotate = ((Input.GetKey(KeyCode.Q) ? -30 : 0) + (Input.GetKey(KeyCode.E) ? 30 : 0)) * Time.deltaTime;
            float _camUpDown = ((Input.GetKey(KeyCode.F) ? -1 : 0) + (Input.GetKey(KeyCode.R) ? 1 : 0));

            me.Rotate(0, 0, _camRotate);
            moveSpeed *= Input.GetAxis("Mouse ScrollWheel") + 1;

            if (followObject)
            {
                followVector += (Input.GetAxis("Horizontal") * me.right + Input.GetAxis("Vertical") * me.forward + me.up * _camUpDown).normalized * moveSpeed * Time.deltaTime;
                me.position = followObject.position + followVector;
            }
            else
            {
                me.position += (Input.GetAxis("Horizontal") * me.right + Input.GetAxis("Vertical") * me.forward + me.up * _camUpDown).normalized * moveSpeed * Time.deltaTime;
            }
